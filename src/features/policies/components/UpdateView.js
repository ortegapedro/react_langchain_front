...
import { getPolizas, updatePoliza } from '../services/poliza';
...
function UpdateView({ onSessionExpired }) {
  ...
  const handleSave = async (id) => {
    ...
    try {
      ...
      if (err.code === 'SESSION_EXPIRED') onSessionExpired();
      else setError('Failed to update policy.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <ContentCard title="Update Policy">
      ...
      {loading ? (
        ...
        <table className="crud-table">
          <thead>
            <tr><th>#</th><th>Client</th><th>Policy Number</th><th>Action</th></tr>
          </thead>
          <tbody>
            {polizas.map(p =>
              editingId === p.id ? (
                <tr key={p.id}>
                  <td>{p.id}</td>
                  <td><input value={editValues.cliente} onChange={e => setEditValues(v => ({ ...v, cliente: e.target.value }))} autoFocus /></td>
                  <td><input value={editValues.numero_poliza} onChange={e => setEditValues(v => ({ ...v, numero_poliza: e.target.value }))} /></td>
                  <td>
                    <button className="btn-save" onClick={() => handleSave(p.id)} disabled={saving}>{saving ? 'Saving...' : 'Save'}</button>
                    <button className="btn-cancel" onClick={cancelEdit} disabled={saving}>Cancel</button>
                  </td>
                </tr>
              ) : (
                <tr key={p.id}>
                  <td>{p.id}</td><td>{p.cliente}</td><td>{p.numero_poliza}</td>
                  <td><button className="btn-edit" onClick={() => startEdit(p)}>Edit</button></td>
                </tr>
              )
            )}
          </tbody>
